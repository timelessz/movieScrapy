        ...
        if 'name' in item.keys() and item['name'] != '':
            ...
            with self.conn.cursor() as cursor:
                ...
                for movietype in movietype_info:
                    ...
                    if type_id == 0:
                        ...
                        currenttime = int(time.time())
                        typesql = 'insert into movie_type(`name`,`create_time`) VALUES ("%s","%s")' % (
                ...
                cursor.execute(sql, (
                    ...
                    item['length'], item['starring'], item['director'], item['summary'], pymysql.escape_string(item['content']),
                    item['region_id'], \
                    item['region_name'], currenttime, currenttime, item['href'], coversrc))
                self.conn.commit()
            ...
            for imgsrc in item['imglist']:
                if not coversrc:
                    ...
                    imglist.append(imgsrc)
            with self.conn.cursor() as cursor:
                sql = "insert into movie.movie_xunleipu_original_list(title, content, region_id, region_name, update_time, create_time, href, coversrc) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)"
                cursor.execute(sql, (
                    item['title'], pymysql.escape_string(item['content']), item['region_id'], item['region_name'],
                    currenttime, currenttime, item['href'], coversrc))
                self.conn.commit()
                selectsql = 'select id,title from movie.movie_xunleipu_original_list WHERE title="%s"' % item['title']
                cursor.execute(selectsql)
                movie_idinfo = cursor.fetchone()
                print(movie_idinfo)
                # 添加电影的下载链接 这块可以单独封装函数
                add_download_sql = 'insert into movie.movie_xunleipu_original_download_link(movie_id, type_name, type_id,href, text, pwd, create_time, update_time) VALUES '
                templatesql = "(%s,'%s',%s,'%s','%s','%s',%s,%s)"
                insert_sql = ''
                currenttime = int(time.time())
                i = 1
                ...
                if insert_sql:
                    ...
                    self.conn.commit()
                # 然后添加电影的图片链接 这块可以单独封装函数
                add_img_sql = 'insert into movie.movie_xunleipu_original_imglist(movie_id,imgsrc,create_time,update_time) VALUES'
                imgtemplatesql = "(%s,'%s',%s,%s)"
                insert_img_sql = ''
                currenttime = int(time.time())
                i = 1
